...
class ThreadRepositoryPostgres extends ThreadRepository {
  ...
  async findThreadById(threadId) {
    const threadQuery = {
      text: `SELECT threads.id AS id, threads.title AS title, threads.body AS body, threads.date AS date, users.username AS username
    ...
    const commentQuery = {
      text: `SELECT comments.id AS id, users.username AS username, comments.date AS date, comments.content AS content, comments.is_delete AS "isDelete" 
                FROM comments INNER JOIN users on comments.owner = users.id WHERE comments.thread = $1 ORDER BY date ASC`,
      values: [threadId],
    };
